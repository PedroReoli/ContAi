...
import { calcularIcmsOperacao, calcularIcmsDiferido, calcularIcms51 } from "@/utils/calculadoraICMS"
...
const CalculadoraICMS51: React.FC<CalculadoraProps> = ({ storageDisponivel }) => {
  ...
  const calcular = () => {
    const valores = {
      ...
      percentualDiferimento: parseNumberBR(formData.percentualDiferimento || "0"),
    ...
    const icmsDiferido = calcularIcmsDiferido(
      ...
      valores.valorDesconto,
      valores.aliquotaIcms,
      valores.percentualDiferimento,
      valores.valorIpi,
    )

    const valorIcms = calcularIcms51(
      valores.valorProduto,
      valores.valorFrete,
      valores.valorSeguro,
      valores.valorOutro,
    ...
    setResultado({ icmsOperacao, icmsDiferido, valorIcms })
  }
  ...
  const salvar = () => {
    ...
    try {
      ...
      const novoCalculo = salvarCalculo({
        ...
        valorBase: resultado.icmsOperacao,
        aliquota: valores.aliquotaIcms,
        resultado: resultado.valorIcms,
        detalhes: {
          ...valores,
          icmsOperacao: resultado.icmsOperacao,
          icmsDiferido: resultado.icmsDiferido,
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Input
              ...
              value={formData.percentualDiferimento}
              onChange={handleChange}
              placeholder="0,00"
              className="bg-white dark:bg-bg-darker border-bg-medium/30 dark:border-bg-dark/50 rounded-xl p-4 focus-within:ring-2 focus-within:ring-primary/30 dark:focus-within:ring-secondary/30 transition-all"
            />
            ...
            <div className="mt-8 p-6 bg-bg-light/50 dark:bg-bg-darker/50 backdrop-blur-sm rounded-xl border border-bg-medium/30 dark:border-bg-dark/50 shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-bg-darker p-5 rounded-xl border border-bg-medium/30 dark:border-bg-dark/50 shadow-sm hover:shadow-md transition-shadow">
                  <p className="text-sm text-txt-secondary dark:text-txt-muted">ICMS da Operação:</p>
                  <p className="font-medium text-xl text-txt-primary dark:text-txt-light mt-1">
                    {formatCurrency(resultado.icmsOperacao)}
                  </p>
                </div>

                <div className="bg-white dark:bg-bg-darker p-5 rounded-xl border border-bg-medium/30 dark:border-bg-dark/50 shadow-sm hover:shadow-md transition-shadow">
                  <p className="text-sm text-txt-secondary dark:text-txt-muted">ICMS Diferido:</p>
                  <p className="font-medium text-xl text-txt-primary dark:text-txt-light mt-1">
                    {formatCurrency(resultado.icmsDiferido)}
                  </p>
                </div>
                ...
                <div className="relative overflow-hidden bg-white dark:bg-bg-darker p-5 rounded-xl border-2 border-primary/20 dark:border-secondary/20 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="relative">
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-txt-secondary dark:text-txt-muted">Valor do ICMS a Recolher:</p>
...
export default CalculadoraICMS51
